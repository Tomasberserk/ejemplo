 ...
 * Permite almacenar registros de asistencia en localStorage cuando no hay señal
 * y sincronizarlos de forma automática e idempotente al recuperar la conexión.
 */
...
export class OfflineAttendanceQueue {
   ...
   * @param {Object} checkinData Datos de la marcación (token, documento, photo_evidence, etc.)
  ...
  enqueue(checkinData) {
    ...
    console.log('[OfflineQueue] Marcación encolada:', item.id);
    return item;
  }
  ...
  async syncAll(authToken = '') {
    if (this.isSyncing) {
      console.log('[OfflineQueue] Sincronización en curso...');
      return { synced: 0, failed: 0 };
    }

    if (!navigator.onLine) {
      console.log('[OfflineQueue] Sin conexión. No se puede sincronizar.');
      return { synced: 0, failed: 0 };
    }
    ...
    for (const item of queue) {
      try {
        ...
        if (response.ok || response.status === 200 || response.status === 201) {
          synced++;
          console.log('[OfflineQueue] Marcación sincronizada exitosamente:', item.id);
        } else {
          const errData = await response.json().catch(() => ({}));
          console.warn('[OfflineQueue] Error del servidor al sincronizar:', errData);
          // Si es un error de cliente irrecuperable (ej: documento no existe), o si ya superó 5 reintentos
        ...
        console.warn('[OfflineQueue] Error de red durante la sincronización:', err);
        item.retries = (item.retries || 0) + 1;
        remainingQueue.push(item);
        failed++;
        break; // Detener el ciclo si falló la conexión
      }
    }
  ...
  /**
   * Suscribe un callback al finalizar la sincronización
   * @param {Function} cb 
   */
  onSyncComplete(cb) {
  ...
  _initListeners() {
    if (typeof window !== 'undefined') {
      window.addEventListener('online', () => {
        console.log('[OfflineQueue] Conexión restablecida. Iniciando sincronización automática...');
        this.syncAll();
      });

      // Intento de sincronización periódico cada 30 segundos si hay internet
      setInterval(() => {
